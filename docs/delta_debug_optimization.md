...
> **Définition 3 (Propriété ($\star$)) :** Soit $(E, T)$ un problème, $n\in\mathbb{N}^*$ et $b_1,...,b_n\in\mathcal{P}(E)$, on dit que $B = \{b_1,...,b_n\}$ verifie ($\star$) ou que $b_1,...,b_n$ vérifient ($\star$) si $$\forall i \neq j \in ⟦ 1, n⟧, b_i \nsubseteq b_j \text{ et } \forall i \in ⟦ 1, n⟧, b_i \neq \varnothing $$

On remarque que cette définition est symétrique quitte à inverser i et j.

> **Propriété 1 (Équivalence atomique)**  
> Soit $(E, T)$ un problème, il existe un unique $n \in \mathbb{N}^*$  
> et $b_1,\dots,b_n \in \mathcal{P}(E)$ vérifiant $(\star)$ tels que :
>
> $$
> \forall C \in \mathcal{P}(E), \quad 
> T(C) = ✗ \iff 
> \exists i \in ⟦ 1, n \rbrack\!\rbrack : b_i \subseteq C
> $$
>
> On dit alors que $b_1,\dots,b_n$ sont les atomes du problème.  
> De plus, l'application :
>
> $$
> f : T \mapsto \{b_1,\dots,b_n\}
> \quad \in \quad
> \{\{b_1,\dots,b_n\} \in \mathcal{P}(E)^n \;|\;
> n \in \mathbb{N}^*,\; \{b_1,\dots,b_n\} \text{ vérifient } (\star)\}
> $$
>
> est une bijection de l'espace des fonctions test vers l'ensemble des familles d'atomes.
...
>      - Par hypothèse de récurrence on a $i \in ⟦ 1, n⟧$ tel que $b_i\subseteq A$ et donc par transitivité de l'inclusion on a $b_i\subseteq C$, ce qui clot la récurrence.
>
...
>- Soit $i\in ⟦ 1, m⟧$, comme $c_i$ est un atome, on a $T(c_i) = ✗$
>- Or comme $b_1,...,b_n$ sont aussi les atomes du problème on a :
>   - $j\in ⟦ 1, n⟧$ tel que $b_j \subseteq c_i$
>- Or tout comme pour $c_i$, on a $T(b_j) = ✗$
>   - Donc on a $k\in ⟦ 1, n⟧$ tel que $c_k \subseteq b_j$
...
>
> Réciproquement si on prend $n\in\mathbb{N}^*$ et $b_1,...,b_n\in \mathcal{P}(E)$ vérifiant ($\star$) alors on peut bien définir une fonction test $T$ sur $E$, en posant $\forall C \in \mathcal{P}(E), T(C) = ✗ \iff \exists i \in ⟦ 1, n⟧: b_i \subseteq C$, on obtient bien une fonction croissante (par transitivité de l'inclusion), les deux autres conditions sont triviales étant donnée la non nullité de n et des $b_i$ fournie par ($\star$)\
...
>
> $$ \forall i \in ⟦ 1, n ⟧, (|\Delta_i| = 1\text{ et } \exists i_0 \in ⟦ 1, m ⟧: \Delta_i = \delta_{i_0})\text{ ou }(\exists i_0, j_0\in ⟦ 1, m ⟧: \delta_{i_0} \sqcup \delta_{j_0} = \Delta_i)$$
> On définit alors la fonction de correspondance $f: ⟦ 1, n ⟧ \longrightarrow \mathcal{P}_1(⟦ 1, m ⟧)\cup \mathcal{P}_2(⟦ 1, m ⟧)$ telle que $$\forall i\in ⟦ 1, n ⟧, \Delta_i = \bigcup_{j \in f(i)} \delta_j$$

> ***Remarque :*** Cette définition est potentiellement un peu désagréable à digérer mais ce qu'il faut comprendre c'est que $\delta$ est la même partition que $\Delta$ où l'on a coupé chaque partie en deux d'où : $\exists i_0, j_0\in ⟦ 1, m ⟧: \delta_{i_0} \sqcup \delta_{j_0} = \Delta_i$, sauf dans le cas où $\Delta_i$ était de cardinal 1 au quel cas on l'a simplement conservé, d'où : $|\Delta_i| = 1\text{ et } \exists i_0 \in ⟦ 1, m ⟧: \Delta_i = \delta_{i_0}$
...
> $$ \forall i \in ⟦ 1, n-1 ⟧, \Delta^{i+1} \text{ raffine } \Delta^i$$
...
> $$\forall i,j \in ⟦ 1, n ⟧, \big| |\Delta_i| - |\Delta_j|\big| \leq 1$$
...
> $$\Big\{\{k\in \Delta\text{ | } B_i \cap \Delta_k \neq \varnothing \}\text{ } \Big | \text{ } i \in ⟦ 1, card(B)⟧\Big\}

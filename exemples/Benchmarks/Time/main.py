...
if __name__ == "__main__":
    ...
    with pkl_path.open("rb") as f:
        while True:
            try:
                size, problem, waiting_time, force, tag = pickle.load(f)
                pre_test.append([size, problem, waiting_time, force, tag])
            except EOFError:
                break
    ...
    to_test = []
    for size, problem, waiting_time, force, tag in pre_test:
        if [size, problem, waiting_time] not in done_norm or force:
            ...
            if [size, problem, waiting_time] not in to_test:
                to_test.append([size, problem, waiting_time, tag])
    ...
    for size, problem, waiting_time, tag in to_test:
        print(f"to_launch : size={size}, waiting_time={waiting_time}, problem={problem}, tag={tag}")
    counter = 1
    for size, problem, waiting_time, tag in to_test:
        with open("data/lockcell_bench.pkl", "ab") as file:
            ...
            pickle.dump([size, problem, waiting_time, execution_time, updates, tag], file)
            print(
                f"run {counter}/{n} :done : size={size}, waiting_time={waiting_time}, problem={problem}, execution_time={execution_time}, tag={tag}",
                "\n",
                "-" * 150,
            )
            counter += 1
